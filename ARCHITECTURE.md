...
| Собственный `contact.php` (PHP + php-fpm) | приём формы | до 12.09.2026 был Formspree; заменён при переезде на VPS ради 152-ФЗ — ПДн теперь не покидают собственный сервер даже транзитом через сторонний SaaS |
...
Отправка — AJAX `fetch()` на `action="/contact.php"`, собственный эндпоинт на том же сервере (см. `main.js`, обработчик `submit`; до 12.09.2026 форма шла на Formspree — см. раздел 13). Перед отправкой JS вручную проверяет чекбокс согласия (на случай обхода браузерной валидации). Во время запроса кнопка получает `data-state="loading"` (блокирует повторный submit через `pointer-events:none` в CSS + JS-флаг).

**Список полей в таблице выше не актуализирован после переезда** — текущий фактический набор полей описан в `CLAUDE.md` (раздел «Форма обратной связи»), там же контракт `server/contact.php`.
...
CSP и остальные security-заголовки применяются как **настоящие HTTP-заголовки** (не `<meta http-equiv>` — meta игнорирует `frame-ancestors` и `form-action`, поэтому только так). С 12.09.2026 источник правды — `/etc/nginx/snippets/security-headers.conf` на сервере; `public/_headers` в репозитории держится в синхроне как справочник (Cloudflare Pages их больше не читает — сайт там не хостится):
...
**12.09.2026 сайт переехал с Cloudflare Pages на собственный VPS** — основная причина: 152-ФЗ (форма и хостинг должны обрабатывать ПДн россиян на серверах в РФ), см. `TECHDEBT.md`.

- **Основной**: VPS FirstByte, Москва (`157.228.191.32`, Ubuntu 24.04, nginx + php-fpm). Push в `main` → GitHub Actions (`.github/workflows/deploy-vps.yml`): `npm run build`, затем `rsync` по SSH `dist/` в `/var/www/a-bra.ru` на сервере
  - Деплоит отдельный непривилегированный системный пользователь `deploy` (не root, без sudo, доступ только на запись в `/var/www/a-bra.ru`) — принцип наименьших привилегий: утечка деплой-ключа из GitHub Secrets даёт максимум подмену статики, не доступ к серверу целиком. Ключ администратора (root, для nginx/TLS/пакетов) отдельный и никогда не покидает машину владельца
  - `server/contact.php` в репозитории — зеркало того, что выполняется на сервере как `/opt/abra/contact.php`. CI его не трогает: исполняемый серверный код сознательно деплоится вручную (`scp` + `php -l` для проверки синтаксиса), а не тем же автоматическим пайплайном, что и статика
  - nginx-конфиг и TLS-сертификат (Let's Encrypt, автопродление через systemd-таймер) — состояние сервера, не репозитория
- **Резервный (fallback)**: GitHub Pages, не связан с переездом на VPS, работает как раньше. Переключается через переменную окружения `DEPLOY_TARGET=gh-pages`, которая меняет `base` в `vite.config.js` с `/` на `/abra/` (нужно для корректных путей на project pages URL вида `username.github.io/abra/`). Держится осознанно как запасной вариант — свой собственный workflow, `.github/workflows/deploy.yml`
- После каждого push в `main` стоит проверять, что VPS реально выкатил новую версию, а не полагаться на факт пуша:
  ```bash
  npm run build
  curl -s https://a-bra.ru/ | grep -o 'assets/main-[^"]*\.js'
  # сравнить хэш с тем, что лежит в dist/assets после локальной сборки
  # или посмотреть последний прогон: gh run list --workflow deploy-vps.yml --limit 1
  ```
...
- **152-ФЗ — техническая часть закрыта 12.09.2026**: хостинг и форма переехали на собственный VPS в Москве. Юридическая часть (уведомление в Роскомнадзор, пересмотр Политики конфиденциальности, доменная почта вместо временного Gmail-адреса для приёма заявок) остаётся открытой — см. `TECHDEBT.md`.
- На DNS-зоне `a-bra.ru` есть независимый второй проект — `lab.a-bra.ru` (не в этом репозитории). Переезд его не затронул, но перед любыми будущими действиями с этой DNS-зоной — прочитать соответствующий раздел `TECHDEBT.md`.
